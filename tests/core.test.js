...
test('running and upcoming services',()=>{const r={freq:{wd:{'0800':null}},jt:'60'};assert.equal(serviceStatus(r,data,new Date('2026-09-15T00:45Z')).kind,'active');assert.equal(serviceStatus(r,data,new Date('2026-09-14T23:50Z')).kind,'upcoming')});
...
test('legacy schedules use the same fixed early allowance',()=>{
 const route={freq:{wd:{'1200':null}},jt:'120'};
 assert.equal(serviceStatus(route,data,new Date('2026-09-15T03:49:59Z')).kind,'inactive');
 assert.equal(serviceStatus(route,data,new Date('2026-09-15T03:50:00Z')).kind,'upcoming');
});
